...
def main():
    print("=" * 50)
    print("CORRECTION DES TRADUCTIONS - VERSION PYTHON")
    print("=" * 50)
    print()
    ...
    for po_file in po_files:
        lang = po_file.split('/')[1]
        if update_po_file(po_file, lang):
            updated.append(lang)
            print(f"      ✅ Mis à jour")
    
    print()
    print("Compilation des fichiers .mo...")
    for po_file in po_files:
        lang = po_file.split('/')[1]
        mo_file = po_file.replace('.po', '.mo')
        print(f"   {lang}... ", end='')
        result = subprocess.run(['msgfmt', '-o', mo_file, po_file], capture_output=True)
